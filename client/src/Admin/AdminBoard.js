...
//import { userActions } from "../redux/akcije/korisnicke.akcije";
...
export default class AdminBoard extends React.Component {
  ...
  handleUplatu(id) {
    userService.odradiUplatu(id).then((res) => {
      console.log(res);
    });
  }
  ...
  handleSpremanjeOklade() {
    ...
    userService.spremiNovuOkladu(oklada).then((res) => {
      alert(res.poruka);
    });
  }
  izmjeniBrojIshoda(name) {
    if (name === "minus") {
      this.setState({ brojIshoda: this.state.brojIshoda - 1 });
    }
    if (name === "plus") {
      this.setState({ brojIshoda: this.state.brojIshoda + 1 });
    }
        ...
        <div>
          <div>
            ...
            <input
              ...
              name={index}
              value={oklada.sanse[index]}
              onChange={this.izmjeniSanse}
            />
          </div>
          ...
          <input
            ...
            className="form-control"
            name={index}
            value={oklada.tipovi[index]}
            onChange={this.izmjeniTipove}
          />
        </div>
      );
    }
  ...
  test() {
    // userService.test(user).then(res=>alert(res.poruka))
  }

  render() {
    const { uplate, isAdmin, oklada, oklade } = this.state;
      ...
      <div>
                ...
                <div>
                  <li key={oklade._id}>
                    ...
                    <span>
                      ...
                      <button onClick={() => this.izracunajOkladu(oklade)}>
                        izračunaj
                      </button>
                    </span>
                  </li>
                  ...
                  {!uplate.odradeno && (
                    <li key={uplate._id}>
                      ...
                      <span>
                        {" "}
                        -{" "}
                        <button onClick={() => this.handleUplatu(uplate._id)}>
                          Uplati
                        </button>
                        <button
                          onClick={() => this.handleBrisanjeUplate(uplate._id)}
                        >
                          Obrisi
                        </button>
                      </span>
        ...
        <div className="group">
          ...
          <input
            ...
            className="form-control"
            name="opisOklade"
            value={oklada.opisOklade}
            onChange={this.handleChange}
          />
        </div>
        {this.state.brojIshoda < 4 && (
          <button name="plus" onClick={() => this.izmjeniBrojIshoda("plus")}>
            +
          </button>
        )}
        <span>broj ishoda:{this.state.brojIshoda}</span>
        {this.state.brojIshoda > 1 && (
          <button name="minus" onClick={() => this.izmjeniBrojIshoda("minus")}>
            -
          </button>
        )}
        ...
        <div className="group">
          <button
            onClick={() => this.handleSpremanjeOklade()}
