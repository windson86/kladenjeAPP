...
exports.dodavanjeOklade = (req,res) =>{
  console.log(req.body)
   const oklada= new OkladaDvaTip(req.body)
    oklada.save()
}
...
exports.uplatiNaRacun = (req,res) =>{
    ...
    ZahtjevUplate.findOneAndUpdate({_id:req.body.id},{$set:{odradeno:true}},{new:true},(err,uplata)=>{
      if(err){res.status(500).send({message : err});return;}
      uplata.save().then(
...
exports.signup = (req,res) =>{
    const data = {...req.body,password:bcrypt.hashSync(req.body.password, 10)}
...
exports.isAdmin = (req,res)=>{
  ...
  Kladitelj.findOne({_id:req.body.id})
  .exec((err,kladitelj)=>{
    if(err){res.status(500).send({message : err});return;}
   if(kladitelj){
...
exports.signin= (req,res) => {
            Kladitelj.findOne({username: req.body.username})
            .exec((err,kladitelj)=>{
